...
#Train Set
SRC_FILE=../../data/learner/CCL2023/track1/train/train_clean/simplified/lang8.train.ccl22.src.clean.simple  # 每行一个病句
TGT_FILE=../../data/learner/CCL2023/track1/train/train_clean/simplified/lang8.train.ccl22.tgt.clean.simple  # 每行一个正确句子，和病句一一对应
if [ ! -f $SRC_FILE".char" ]; then
    python ../../tools/segment/segment_bert.py --data_file $SRC_FILE --char_file $SRC_FILE".char"  # 分字
fi
if [ ! -f $TGT_FILE".char" ]; then
...
## Generate label file
LABEL_FILE=../../data/learner/CCL2023/track1/train/train_clean/simplified/lang8.train.ccl22.clean.simple.label  # 训练数据
if [ ! -f $LABEL_FILE ]; then
    python ./utils/preprocess_data.py \
     -s $SRC_FILE".char" \
     -t $TGT_FILE".char" \
...
MODEL_DIR=./exps/seq2edit_lang8_CCL
if [ ! -d $MODEL_DIR ]; then
  mkdir -p $MODEL_DIR
fi
...
#
#INPUT_FILE=../../data/MuCGEC_exp_data/MuCGEC_exp_data/test/MuCGEC.input # 输入文件
