...
def save_checkpoint(args, trainer, epoch_itr, val_loss):
    ...
    checkpoint_conds["checkpoint{}{}.pt".format(epoch, suffix)] = end_of_epoch and not args.no_epoch_checkpoints and epoch % args.save_interval == 0
    checkpoint_conds["checkpoint_{}_{}{}.pt".format(epoch, updates, suffix)] = (
        not end_of_epoch and args.save_interval_updates > 0 and updates % args.save_interval_updates == 0
    )
    ...
    if val_loss is not None and args.keep_best_checkpoints > 0:
        checkpoint_conds["checkpoint.best_{}_{:.2f}.pt".format(args.best_checkpoint_metric, val_loss)] = not hasattr(save_checkpoint, "best") or is_better(
    ...
    if len(checkpoints) > 0:
        ...
        write_timer.stop()
        logger.info(
            "saved checkpoint {} (epoch {} @ {} updates, score {}) (writing took {} seconds)".format(checkpoints[0], epoch, updates, val_loss, write_timer.sum)
        )
    ...
    if args.keep_best_checkpoints > 0:
        ...
        checkpoints = checkpoint_paths(
            args.save_dir,
            pattern=r"checkpoint\.best_{}_(\d+\.?\d*)\.pt".format(args.best_checkpoint_metric),
        )
...
def load_checkpoint(args, trainer, **passthrough_args):
    ...
    *passthrough_args* will be passed through to
    ...
    if getattr(args, "finetune_from_model", None) is not None and (reset_optimizer or reset_lr_scheduler or reset_meters or reset_dataloader):
        raise ValueError(
    ...
    if args.restore_file != "checkpoint_last.pt" and getattr(args, "finetune_from_model", None):
        raise ValueError("--finetune-from-model and --restore-file (non-default value) " "can not be specified together: " + str(args))
