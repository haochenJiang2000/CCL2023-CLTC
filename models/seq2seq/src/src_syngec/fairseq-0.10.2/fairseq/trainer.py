    ...
    def load_checkpoint(
        ...
        if getattr(self.args, "syntax_model_file", None) is not None:
            ...
            if bexists:
                ...
                self.get_model().load_syntax_state_dict(state["model"], strict=True, args=self.args)
            logger.info("loaded syntax-enhanced-encoder from " + gat_filename)
        ...
        if getattr(self.args, "bart_model_file_from_transformers", None) is not None:
            ...
            self.get_model().load_bart_state_dict_from_transformers(model, strict=True, args=self.args)
            logger.info("loaded bart parameters from " + bart_model_file_from_transformers)
    ...
    def get_train_iterator(
        ...
        batch_iterator = self.task.get_batch_iterator(
            ...
            max_positions=utils.resolve_max_positions(
                self.task.max_positions(), self.model.max_positions(), self.args.max_tokens, self.args.max_sentence_length
            )
            if hasattr(self.args, "max_sentence_length")
            else utils.resolve_max_positions(self.task.max_positions(), self.model.max_positions(), self.args.max_tokens),
        ...
        if not overflow or self.args.distributed_wrapper == "SlowMo":
            ...
            if self.tpu:
                ...
                if (
                    ...
                    and ((self.get_num_updates() + self.args.empty_cache_freq - 1) % self.args.empty_cache_freq) == 0
                ):
                    torch.cuda.empty_cache()
    ...
    def _check_grad_norms(self, grad_norm):
        ...
        if self._grad_norm_buf is not None:
            ...
            def is_consistent(tensor):
                ...
                return torch.isfinite(tensor).all() or (max_abs_diff / (tensor[0] + 1e-6) < 1e-6).all()
    ...
    def _check_xla_compilation(self):
        ...
        if num_xla_compiles > self._num_xla_compiles:
            logger.warning(
                ...
                "to slow training, but we expect a few in the beginning".format(self.args.distributed_rank)
            )
